...
# 2. Проверка что добавили товар в корзину
    ...
    time.sleep(1)
    assert (driver.current_url == "https://www.saucedemo.com/cart.html"), "Не зашли в корзину"

    css_loc = ".cart_list > .cart_item .inventory_item_name"
    item_name = driver.find_element(By.CSS_SELECTOR, css_loc).text
    time.sleep(1)
    assert (item_name == "Test.allTheThings() T-Shirt (Red)"), "В корзине нет нужного товара"
